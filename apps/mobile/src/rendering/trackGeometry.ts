...
function snapshotSegments(
  ...
  return [{ explorationId: snapshot.explorationId, track: snapshot.track }];
}
...
function opacityBucket(confidence: number): number {
  const visible = Math.max(MINIMUM_VISIBLE_OPACITY, Math.min(1, confidence));
  const bucketed =
...
function buildSegmentStrokes(
  ...
  function finishActiveStroke(): void {
    if (activeBand === null || activePoints.length < 2) {
      activePoints = [];
      return;
    }
    strokes.push({
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    if (activeBand === edgeBand && activeOpacity === edgeOpacity) {
...
export function buildTrackCanvasGeometry(input: {
  ...
  if (width <= 0 || height <= 0 || input.snapshot.bounds === null) {
    return {
      width,
      height,
      pointCount,
  ...
  const markers = input.snapshot.markers.flatMap((marker) => {
    ...
    return [
      {
        marker,
        point: projectPoint(marker.xMeters, marker.yMeters, projection),
      },
    ];
  });
