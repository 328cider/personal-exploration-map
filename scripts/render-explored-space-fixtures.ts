...
const ROW_STRIDE = PANEL_HEIGHT + 60;
const MAP_WIDTH = 350;
const MAP_HEIGHT = 214;
const OUTER_PADDING = 24;
const FIRST_PANEL_TOP = 124;
...
function outAndBackFixture(): Fixture {
  ...
  return {
    id: "out-and-back",
    title: "Out and back",
    question: "Does overlap read as one passage rather than a wider road?",
...
function plazaSweepFixture(): Fixture {
  ...
  return {
    ...
    question: "Do nearby passages accumulate without claiming a surveyed polygon?",
    segments: [
      {
        id: "plaza-session",
...
function sparseMixedAccuracyFixture(): Fixture {
  return {
    ...
    question: "Is uncertainty visible without expanding passage-cell area?",
    segments: [
      {
        id: "sparse-session",
        points: [
...
function overlappingSessionsFixture(): Fixture {
  ...
  return {
    ...
    question: "Does a second session strengthen evidence without being required?",
    segments: [
...
function withUniformAccuracy(
  fixture: Fixture,
  accuracy: number,
): readonly ExploredSpaceSegment[] {
...
function renderPanel(
  ...
  const offsetY = FIRST_PANEL_TOP + row * ROW_STRIDE;
  ...
  const maximumSupportingSessions = geometry.cells.reduce(
    (maximum, cell) => Math.max(maximum, cell.supportingSessions),
    0,
  );
  const averageCellConfidence = average(
    geometry.cells.map((cell) => cell.confidence),
  );
  ...
  const metric =
    mode === "uncertainty"
      ? `${geometry.uncertaintyBands.length} uncertainty primitives`
      : mode === "cells"
        ? `${geometry.cells.length} cells · ${Math.round(geometry.cellSizeMeters ?? 0)}m · max ${maximumSupportingSessions} sessions`
        : `${renderedPointCount} rendered points`;

  const svg = `<g transform="translate(${offsetX} ${offsetY})">
    <rect x="0" y="0" width="${PANEL_WIDTH - 18}" height="${PANEL_HEIGHT}" rx="18" fill="#ffffff" stroke="#dce2dd"/>
...
function validateMatrix(results: readonly FixtureModeResult[]): void {
  ...
  const singleSessionResults = results.filter(
    (result) =>
      result.mode === "cells" && result.fixtureId !== "overlapping-sessions",
  );
  if (singleSessionResults.some((result) => result.maximumSupportingSessions !== 1)) {
    throw new Error(
      "Dense observations in one session were incorrectly counted as revisits.",
    );
  }
  ...
  if (revisit === undefined || revisit.maximumSupportingSessions < 2) {
    throw new Error("A second session did not strengthen any passage cell.");
  }
  ...
  if (uncertaintyWidths.size < 2) {
    throw new Error("Mixed accuracy did not produce visibly different uncertainty widths.");
  }

  const accurateSegments = withUniformAccuracy(sparseFixture, 4);
  const poorSegments = withUniformAccuracy(sparseFixture, 30);
  const sharedBounds = boundsFor(sparseFixture.segments);
  const accurateGeometry = buildExploredSpaceGeometry({
    segments: accurateSegments,
    bounds: sharedBounds,
    width: MAP_WIDTH,
    height: MAP_HEIGHT,
    padding: 24,
  });
  const poorGeometry = buildExploredSpaceGeometry({
    segments: poorSegments,
    bounds: sharedBounds,
    width: MAP_WIDTH,
    height: MAP_HEIGHT,
    padding: 24,
  });
  const accurateIds = accurateGeometry.cells.map((cell) => cell.id);
  const poorIds = poorGeometry.cells.map((cell) => cell.id);
  if (JSON.stringify(accurateIds) !== JSON.stringify(poorIds)) {
    throw new Error(
      "Poor horizontal accuracy expanded or changed the passage-cell footprint.",
    );
  }
  if (
    average(poorGeometry.cells.map((cell) => cell.confidence)) >=
    average(accurateGeometry.cells.map((cell) => cell.confidence))
  ) {
    throw new Error(
      "Poor horizontal accuracy did not reduce passage-cell confidence.",
    );
  }
}
...
function renderMatrix(): {
  ...
  const height =
    FIRST_PANEL_TOP +
    ROW_STRIDE * Math.max(0, FIXTURES.length - 1) +
    PANEL_HEIGHT +
    20;
  const panels: string[] = [];
  ...
  for (let row = 0; row < FIXTURES.length; row += 1) {
    ...
    const rowY = FIRST_PANEL_TOP + row * ROW_STRIDE;
    panels.push(
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${OUTER_PADDING}" y="34" font-family="sans-serif" font-size="24" font-weight="900" fill="#17201d">Uncertainty / passage-cell fixture matrix</text>
  <text x="${OUTER_PADDING}" y="58" font-family="sans-serif" font-size="13" fill="#5b6963">Location uncertainty is not explored area or canonical map truth</text>
  ${panels.join("\n")}
</svg>
`;
  return { svg, results };
