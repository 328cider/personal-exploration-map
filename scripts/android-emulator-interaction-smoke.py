...
def assert_tracking_notification(artifacts: Path) -> None:
    output = notification_dump(artifacts, "13-tracking")
...
def open_tracking_notification(artifacts: Path) -> None:
    ...
    smoke.screenshot(artifacts, "13-notification-shade")
    smoke.tap_node(node)
    smoke.wait_for_node(
        artifacts,
        "探索を記録中",
...
def run_interaction_smoke(artifacts: Path) -> dict[str, object]:
    ...
    smoke.screenshot(artifacts, "14-marker-modal")
    ...
    smoke.screenshot(artifacts, "15-marker-saved")
