...
- 欠落、精度、callback、復帰、入力中断を端末内で計測できる
...
- Expo SDK 57のlockfile、Doctor、mobile typecheck、Android development build手順をIssue #2で固定済み
...
diagnostic eventはcanonical map truthではなくbest-effortなoperational evidenceであり、保存失敗でraw位置記録を止めない。詳細はADR 0010に従う。

## 現在の未検証部分

buildと静的検査は成立しているが、次はまだ実端末で十分に確認していない。
...
1. **Issue #3**: diagnostics実装をmainへ反映し、development APKで30〜60分のforeground / background比較を行う
2. **Issue #3**: notification復帰、process recreation、permission変更、battery saver、OEM差を追加測定する
3. **Issue #4**: 10件の実探索でPassive-first UXと「育つ白紙地図」の価値をGo / Narrow / Stop判定する
4. **Issue #17**: 初回tracking開始失敗時に空PersonalMapを残さないcanonical compensationを実装する
...
- 精度未検証PDRの既定有効化
- M0計測前の自動telemetry / analytics service
