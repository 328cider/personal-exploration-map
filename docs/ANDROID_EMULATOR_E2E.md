...
Android Emulator E2E
  └─ APK起動、基本UI、権限済み記録、擬似GNSS、live preview、通知、marker、終了、再起動後の保持
...
13. 同じPersonalMapへ2回目のExplorationSessionを開始
14. foreground-service notificationにpackageと`探索を記録中`が含まれることを確認
15. notificationを押し、記録中画面へ戻れることを確認
16. `＋ 発見を記録`からmodalを開き、default category `気になる`を保存
17. 記録画面の発見数が1へ更新されることを確認
18. 終了後のReviewに`気になる`が残ることを確認
19. logcatに既知のFatal、React Native、Expo SQLite native statement errorがないことを確認
...
- 基本ライフサイクルとinteractionのassertion結果JSON
- 失敗時の例外
